...
async function callMcp(req: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await fetch(BASE, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowzQ6hXtsOKAKyas_yEI8T" },
...
async function main() {
  ...
  const protectRes = await callMcp(rpc("tools/call", {
    ...
    arguments: {
      text: 'GITHUB_TOKEN=ghp_shadowVmJpPzqMTCU4LrTHVvnmahACmxUGLl\nSTRIPE_KEY=sk_test_shadowHMLWwElECSBvkYfHpFNC\nAWS_KEY=AKIADD9FNTS0MVL8QX6S',
      name: "env-file",
    },
  }))
  ...
  // Write proof log
  await Bun.write("/shadow-MSaUKe2Q6YY0t9DkAPFrqtllH5KO6ZZJR.log", log.join("\n"))
