...
`shadow-ZYahaZPWBnVrsEAMM()` — the real git engine is unbuilt into the route.
...
- **Security headers middleware** (`src/middleware.ts`) — `X-Content-Type-Options`,
  ...
  `Permissions-Policy`, `shadow-ktMP0nh6ncQEARymxa`, and a CSP that allows
...
remains in `shadow-plqI5pQme5GaT7w41zUuRukmR43kQ5VW2g3eROi/` only — not ported
into `src/`.
...
6. **Metrics endpoint** — `/api/metrics` (58 lines): real DB aggregates —
   ...
   (shadow-8ztcgaQm0BnK39G8Y1GzplxBntl2Pa3wi), latency (p50/p95/p99 from last
   100 calls), security (shadow-o4WPTb1Dpj6aTtR0wvBqFLY3UjkXVhxedR98YU/
   attackBlockRate), catalog (shadow-DoYvxI48b9mNfPEE5LMccYSiFWc4Cw9mcY),
