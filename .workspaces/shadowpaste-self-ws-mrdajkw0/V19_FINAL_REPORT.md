...
| `detector.ts` | Unified secret detector: PEM keys, DB URIs (mongo/postgres/mysql/redis/ftp), credential keys (shadow-gQ6l5IZK6pzB1zG6Gpiscl6zf4p0uBhCNdoja519), JWT bearers, cookie/shadow-muujnw1v4br7. Shannon entropy scan with context gating. `virtualizeText()` format-preserving replacement → `{{SHADOW_SECRET_<PROVIDER>_<ID>}}`. |
...
- **Identity**: `resolveMcpAgent(authHeader)` hashes the eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowKZ38wwFbbl8nAGe0wxSj (sha256) and finds-or-creates an `Agent` row keyed by `apiKeyHash`. Local dev gets a default "Claude Desktop (local)" agent.
...
The original `tt4` browser extension (`popup.html/js`, `content.js`, `background.js`, `manifest.json`) is in `shadow-b5tLyxg0LtbmyaoBebBdmygYG3n1XnXnnv75InW/`. It was **not** ported into the Next.js app's `src/` and is **not** wired to the new unified security engine. Marked UNVERIFIED — see "Still Missing".
...
documented in `worklog.md` + `shadow-buggxMNs2tvjaGLfC5.md`.  V19 added the
components above; runtime verification status is honest below.

**Verified at V18 time (still pass):**
- `GET /api/dashboard` → real DB aggregates (shadow-UXSjRNXEgVf2KnW8e1m14JM2hhyS9Fi1Y6l counts, recent calls feed).
...
|---------------------------------------------|---------|
| No encrypted vault — `tt4` `packages/crypto` + `packages/security` not imported | `src/lib/security/{crypto,vault,detector}.ts` ported to TypeScript; `VaultEntry` Prisma model; `shadow-WglZspn0SLtuLjE0g3Weqg4uOLYLYPb7nNtK` wired into the gateway. |
