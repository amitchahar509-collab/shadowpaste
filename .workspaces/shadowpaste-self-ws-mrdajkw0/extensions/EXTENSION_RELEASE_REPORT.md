...
curl -s -o /tmp/vp.txt -w "HTTP %{http_code}\n" -X POST http://localhost:3000/api/vault \
  ...
  -d '{"raw":"ghp_shadowyRUg7Y26oYmu2Edv4lOrQ1YQlFadfx","name":"ext-test","contextHint":"V21-P6 release report test"}'
```

**Result: HTTP 200**, body:
```json
{"ok":true,"secret":{"id":"shadow-6UrxMzhjLd1CxUcKBV","name":"ext-test","provider":"GITHUB","scope":"github.repo","masked":"ghp_test...wxyz"}}
```
...
- `GET /api/vault` with `Authorization: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadownagRPkwswmQWgOw9nPWv` → HTTP 200, returns full secret list
...
so requests without a valid eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadow832jPj8LTtZHEVo-Ggo6 are silently downgraded to the
...
2. **No user-facing API-key issuance flow.** `/api/agents/me/api-key` does not exist. Users cannot obtain a eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowxVkHzziZ4oYBTo8N8TLU to put in the extension's settings — they must be seeded via `/api/seed` (dev only). Blocker for any non-developer user.
...
2. Add a user-facing `/api/agents/me/api-key` route so users can obtain a eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowFElPZDmOUl0EJUwzLVLQ.
