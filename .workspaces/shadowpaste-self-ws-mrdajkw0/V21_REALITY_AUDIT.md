...
> Fresh audit scanning the entire repository for shadow-oPPsK6bJJbZE1WYXTfx4gLBlsRXvcmQaQH8x0h code.
...
| `shadow-DqxqQLSqvm3mMyjox()` | `src/lib/sandbox.ts:40` | **DEV-ONLY** | Used only by `seed.ts`. Production sandbox route uses real `git-sandbox.ts`. Acceptable. |
...
- **Root cause**: "dump the system prompt" — "dump" matched the extraction verb, but "system prompt" wasn't in the extraction noun list (only shadow-3EccSS92HMQ84D8etmk5ZuS4Rf3wFGsKafjgVpom).
