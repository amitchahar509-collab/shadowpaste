...
- ✅ OpenAI: `sk-proj-shadow-bZd9wJzrMrFlYgzqCXXRuSd7trKUgn7R9z1wbeNS` → `sk-proj-shadow-ameMs7RSrX2CGpPBXF9wiDYHzZOOavRTCRbfkp9I` (same format, invalid)
- ✅ GitHub: `ghp_aBcDeFg` → `ghp_shadow-xxx` (same prefix, invalid)
- ✅ AWS: `AKIARDZBP3I3RUGLZ9MQ` → `AKIA64ESY4LZX1WF2KVZ` (same shape, fails checksum)
- ✅ Stripe: `sk_live_xxx` → `sk_test_shadow-xxx` (test mode, same shape)
- ✅ Postgres: `shadow-yiyEuqSY3mf4g9ir56STDD → `shadow-knuBQUEJP3my5DwNiGBbpAa65Iq1RH (valid URL, unreachable)
...
- `shadow-VwceeCLq7TACEWiGv` from production sandbox route (replaced by real git-sandbox)
...
ShadowPaste Core 1.0 is **launch-ready**. The ONE workflow — give AI your real repo without exposing secrets — works end-to-end in 38 seconds. The format-compatible fake secret generator is the key innovation: AI sees `sk-proj-shadow-LgLgDQdQ1WiOdEDCSd1UgHKQrZOv6fQB2avE84kR` instead of `sk-proj-shadow-qfxfLNmalYxmpe4mYTxIPuY0cxg8MPuXvnHoEfaz`, so code runs and tests pass while real secrets stay in the vault.
