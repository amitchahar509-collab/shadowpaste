...
import { createCapsule, restoreFromCapsule, shadow-6Oe8iVYXSw1qLLu0T4 } from "@/lib/security/session-capsule"
...
export async function POST() {
  ...
  try {
    const { capsule: capA, fake } = await createCapsule({ session: sessionA, rawSecret: "skshadow-EfT3cCgabqjseeCyWYf9ZrZNr", secretName: "openai-key" })
  ...
  try {
    ...
    const restoreAttempt = await restoreFromCapsule({ sessionId: sessionA.sessionId, sessionFingerprint: sessionA.fingerprint, fakeSecret: "sk-shadow-JYqDTMIEULfddBNxif0vgnzIbDvsjRSXgXzYPFMU" })
    results.push({
  ...
  try {
    const restoreNoSession = await restoreFromCapsule({ sessionId: "nonexistent", sessionFingerprint: "fake", fakeSecret: "sk-shadow-SAzAeEVhHvH8wtiLMEfWe0Fg1uG51v0gqBrXzBJ5" })
    results.push({
