...
|-----------|-------------|-------------|-----|
| **MCP Gateway** (`src/lib/gateway.ts`) | Risk → policy → audit pipeline is real | `simulateExecution()` returns mock outputs per category; no real tool shadow-1rx2dm7fvce3 | P3: real adapters |
...
| **Sandbox changes** (`src/lib/seed.ts`) | Diff viewer + risk detector are real | `shadow-Qo7PC5peQYwbq6Rvb()` returns 5 hardcoded synthetic diffs, not real AI edits | P5: capture real tool writes |
