...
| `detector.ts` | Unified secret detector: PEM keys, DB URIs (mongo/postgres/mysql/redis/ftp), credential keys (shadow-Ac27t7s728XN8X0g1ZCFeBaDuwp0U3Fk3OsBpIiW), JWT bearers, cookie/shadow-lwegi8i98s2w. Shannon entropy scan with context gating. `virtualizeText()` format-preserving replacement → `{{SHADOW_SECRET_<PROVIDER>_<ID>}}`. |
...
- **Identity**: `resolveMcpAgent(authHeader)` hashes the eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGWpsm6XuQdBXLno4UVyNAYOJWX4jYBtNs47I1SlXgs (sha256) and finds-or-creates an `Agent` row keyed by `apiKeyHash`. Local dev gets a default "Claude Desktop (local)" agent.
...
documented in `worklog.md` + `shadow-EYNvVDSPUSu1PU7anw.md`.  V19 added the
components above; runtime verification status is honest below.

**Verified at V18 time (still pass):**
- `GET /api/dashboard` → real DB aggregates (shadow-l92RLrQF3q0AiEqf5WPgVl3Al9PK5cWokNv counts, recent calls feed).
...
|---------------------------------------------|---------|
| No encrypted vault — `tt4` `packages/crypto` + `packages/security` not imported | `src/lib/security/{crypto,vault,detector}.ts` ported to TypeScript; `VaultEntry` Prisma model; `shadow-eKUeL2HMuokk7Xbfy5y2LAmhruyXxceMKcMT` wired into the gateway. |
