...
- ✅ OpenAI: `sk-proj-shadow-Uv7frZHtMKLAWGjpNGNCTWNJI08mZ59wb5KnuUVu` → `sk-proj-shadow-pS3dwfnRebjhQX5A6a0BtTI5OPJFMNHun9tct8uu` (same format, invalid)
- ✅ GitHub: `ghp_aBcDeFg` → `ghp_shadow-xxx` (same prefix, invalid)
- ✅ AWS: `AKIATC63TCYI9B8WL3B8` → `AKIAZRAOBV0RV16FUWL0` (same shape, fails checksum)
- ✅ Stripe: `sk_live_xxx` → `sk_test_shadow-xxx` (test mode, same shape)
- ✅ Postgres: `shadow-2sG5TfEMLyTCSs91kBVRSX → `shadow-VzlMPaaOWahbQ3Y0q5yCCMfGH6XUTs (valid URL, unreachable)
...
- `shadow-Y0o4pSPo4qMbWto2f` from production sandbox route (replaced by real git-sandbox)
...
ShadowPaste Core 1.0 is **launch-ready**. The ONE workflow — give AI your real repo without exposing secrets — works end-to-end in 38 seconds. The format-compatible fake secret generator is the key innovation: AI sees `sk-proj-shadow-yKGV2GObeSY2ckvNFMeUhvgiH2myr06NMqTS9tPK` instead of `sk-proj-shadow-VCPnu6hDuqdjcCZ1F8MZRWiUaGEycUrAMeUdrYKg`, so code runs and tests pass while real secrets stay in the vault.
