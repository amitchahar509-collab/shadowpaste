...
| `detector.ts` | Unified secret detector: PEM keys, DB URIs (mongo/postgres/mysql/redis/ftp), credential keys (shadow-xA3YMa7dKZZ0qpP3TeBvk5Y8C8fPRZsczWu9gelX), JWT bearers, cookie/shadow-zs1fkxc4yh78. Shannon entropy scan with context gating. `virtualizeText()` format-preserving replacement → `{{SHADOW_SECRET_<PROVIDER>_<ID>}}`. |
...
- **Identity**: `resolveMcpAgent(authHeader)` hashes the eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGWpsm6XuQdBXLno4UVyNAYu9f8rGvZW9xfS_3yv6G6pEHFQNokNQZuhEHQ20r6 (sha256) and finds-or-creates an `Agent` row keyed by `apiKeyHash`. Local dev gets a default "Claude Desktop (local)" agent.
...
The original `tt4` browser extension (`popup.html/js`, `content.js`, `background.js`, `manifest.json`) is in `upload/tt4_extracted/SHADOW-100/apps/extension/`. It was **not** ported into the Next.js app's `src/` and is **not** wired to the new unified security engine. Marked UNVERIFIED — see "Still Missing".
...
documented in `worklog.md` + `shadow-kTXp9ZXcyOqdwsH0aG.md`.  V19 added the
components above; runtime verification status is honest below.

**Verified at V18 time (still pass):**
- `GET /api/dashboard` → real DB aggregates (shadow-EPPePe1d0Q3Md9BiPGHsqO8DZiJxkROQrEn counts, recent calls feed).
...
|---------------------------------------------|---------|
| No encrypted vault — `tt4` `packages/crypto` + `packages/security` not imported | `src/lib/security/{crypto,vault,detector}.ts` ported to TypeScript; `VaultEntry` Prisma model; `shadow-npN9cJjZVU3nGQdGQeYpmdj1q4yRTfdcYOCa` wired into the gateway. |
