...
//
// Examples:
//   OpenAI sk-proj-shadow-9hNFFdUQCXDZ0oWVQmSNog3QJarkBBmKMuxOGvJe...  →  sk-shadow-kvyNlbwh4VXkhvoBra6Qt7jFhfsU6Xav0A3UtVeR...  (same shape, clearly fake)
//   shadow-DHjGcSzIsSS3um8SM8 →  shadow-JKbIzKtjxpdeCAadDnuzHFYkctET1 (valid URL, fake creds)
//   AKIAB78J0ZHAPH50RPQF      →  AKIALMEZFNRL1SINP1OF       (same AWS shape, invalid checksum)
...
function rand(len: number, charset = "shadow-hxXmqffDviJHfuHUoCosqJ7px2hGx"): string {
  ...
  switch (provider) {
    case "OPENAI":
      if (raw.startsWith("sk-proj-")) {
        fake = `sk-proj-shadow-MUiXi3JwzGrpCXgiaOzPhMTkkhjRHLyUwI4F9nEH-${rand(40, "shadow-53wsFQqTn0e22TyeQkR5aduJmZmnI3upBZ98jy0q")}`
      } else if (raw.startsWith("sk-ant-")) {
        fake = `sk-antshadow-fNY5HwCXxbhBipPUGvQDnCxz0uPRQC9Ul1d0shadow-5kLJY5i2AqoU8TGTbTIeTZhCgq9HLUBUfs2h2lueG4MYlXYF-${rand(80, "A-Za-z0-9_-")}`
      } else {
        fake = `sk-shadow-${rand(40, "abcdefghijklmnopqrstuvwxyzshadow-SyXdEkjCc9ZzRSHlr8KiMUQGIluxR")}`
      }
      note = "Fake OpenAI key — same format, invalid for API calls, safe for AI to see"
      break

    case "ANTHROPIC":
      fake = `sk-ant-shadow-1q1ghorNOIa46lVbiDREv6HE8UE28CjiStJeHHv5oDOtAEEAh4ilcyzFrpIJHw5s8oO2p46jdCB_X8PsGxj6mnungT-${rand(90, "shadow-EAoUhEgGtZWKaXdvob6ndqcSZAQ0RokRNpGC3K6R-")}`
      note = "Fake Anthropic key — same format, safe for AI"
      break

    case "GITHUB":
      fake = `ghp_shadow${rand(30, "shadow-FAWI0PwNBdnHWSiWtjd6yDcfF7bGsZXODj0T9NTS")}`
    ...
    case "JWT":
      ...
      fake = `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGWpsm6XuQdBXLno4UVyNAYu9f8rGvZW9xfS_3yv6G6WwLYG7DSPjMJtj74S56v.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadow-nd8beRmOAlsfdZxUQ9cyFJ62Z5FehwcJfQl7Sh0K${rand(20, "shadow-wbtifij5OJaMdag0kz4B5jycj5Y0QWS2D0bFtiA0-")}`
...
MIIBshadowRCm/dRTFKm5Yky7UNhMY/Qfs/AZyBhhB
C7QfCOkUTIN8wUCZWxeNLRLjpmO1u02ttm1UV/RSYrf2KGWyUS+5gHpCXzSG/M0fQRFKl3vRDxaUHAgV82zP/WV8SjQrSy
F
P5o/d+R6M7YFc0EELJmcE823iGIIA6hN
GpoYICayHi=0JWVub1H1hRBh5WUQ=g
+r2MIr
-----END SHADOW PRIVATE KEY-----"
    ...
    case "DISCORD":
      fake = `https://discord.com/api/webhooks/${rand(18, "0123456789")}/shadow${rand(40, "shadow-aBLe0iIx2K7TKrXJfkPUz04qi6MRpC6C9fvbcb0H")}`
