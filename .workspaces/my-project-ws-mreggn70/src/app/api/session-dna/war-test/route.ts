...
import { createCapsule, restoreFromCapsule, shadow-bNe5k8q9BmwlbFv27G } from "@/lib/security/session-capsule"
...
export async function POST() {
  ...
  try {
    const { capsule: capA, fake } = await createCapsule({ session: sessionA, rawSecret: "skshadow-IFdxoowtz8xVNGbr3loCIq0cJ", secretName: "openai-key" })
  ...
  try {
    ...
    const restoreAttempt = await restoreFromCapsule({ sessionId: sessionA.sessionId, sessionFingerprint: sessionA.fingerprint, fakeSecret: "sk-shadow-lQIiPUdVm0P1sxfvgukiKZ20GHDCelZHIoUFGUTS" })
    results.push({
  ...
  try {
    const restoreNoSession = await restoreFromCapsule({ sessionId: "nonexistent", sessionFingerprint: "fake", fakeSecret: "sk-shadow-e9Kka1R4YujielbdXlJYmz4HbmyuQWkF9KDzSvXW" })
    results.push({
