...
async function callMcp(req: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await fetch(BASE, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGWpsm6XuQdBXLno4UVyNAYu9f8rGvZW9xfS_3yv6G6yOs_GggyhZfRfubYnWSh" },
...
async function main() {
  ...
  const protectRes = await callMcp(rpc("tools/call", {
    ...
    arguments: {
      text: 'GITHUB_TOKEN=ghp_shadowZCV7pogkFE3HejkCKuM9zBKKhPB8Mm\nSTRIPE_KEY=sk_test_shadow2VpmUnvPgdJw2iaEzWti\nAWS_KEY=AKIABEKNATT7RXSMEDCI',
      name: "env-file",
    },
  }))
