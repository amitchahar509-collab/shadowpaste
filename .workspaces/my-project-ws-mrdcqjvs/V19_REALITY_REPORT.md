...
|-----------|-------------|-------------|-----|
| **MCP Gateway** (`src/lib/gateway.ts`) | Risk → policy → audit pipeline is real | `simulateExecution()` returns mock outputs per category; no real tool shadow-nfecf2h1bfxc | P3: real adapters |
...
| **Sandbox changes** (`src/lib/seed.ts`) | Diff viewer + risk detector are real | `shadow-p9Pr7Dne2UOQm2jed()` returns 5 hardcoded synthetic diffs, not real AI edits | P5: capture real tool writes |
