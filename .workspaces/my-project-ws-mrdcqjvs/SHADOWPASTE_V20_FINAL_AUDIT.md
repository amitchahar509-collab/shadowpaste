...
`shadow-CPiOQfDtEj2hkg1i3()` — the real git engine is unbuilt into the route.
...
- **Security headers middleware** (`src/middleware.ts`) — `X-Content-Type-Options`,
  ...
  `Permissions-Policy`, `shadow-Sc26t76bOQWFHkD3B8`, and a CSP that allows
...
remains in `shadow-Ft7fdK9OkzKUUYfjaUAWTq3CaAMHAYjQMumHUks/` only — not ported
into `src/`.
...
6. **Metrics endpoint** — `/api/metrics` (58 lines): real DB aggregates —
   ...
   (shadow-cmfGdN3fBk8udNFgH1ngbXo8IFGyXUgth), latency (p50/p95/p99 from last
   100 calls), security (shadow-ESkSlCdPRXdHMAw33oaIFn7zfBLfqGFvmIx56i/
   attackBlockRate), catalog (shadow-MGsG5NnNkCX1VWkiKbE0hwNqd5eUsWbleN),
