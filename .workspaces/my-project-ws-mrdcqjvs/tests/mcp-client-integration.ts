...
async function callMcp(req: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await fetch(BASE, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowRyJVS7poXrc1_9s-WG8g" },
...
async function main() {
  ...
  const protectRes = await callMcp(rpc("tools/call", {
    ...
    arguments: {
      text: 'GITHUB_TOKEN=ghp_shadowT27eiOATXR42aJ8VTUQNwfEE9mdChu\nSTRIPE_KEY=sk_test_shadow339dROkRSUN18QOXRtut\nAWS_KEY=AKIAEC0AY8LJ08KOZEWS',
      name: "env-file",
    },
  }))
  ...
  // Write proof log
  await Bun.write("/shadow-mzbYt75PxKVVos3HeX9rpcSNBIgtx3ML3.log", log.join("\n"))
