...
> Fresh audit scanning the entire repository for shadow-ZrnV0gtlh9ThgmkUPGdQ4jK00ViKCBeLrXFCy3 code.
...
| `shadow-hnV9OXInLDGn3ll6K()` | `src/lib/sandbox.ts:40` | **DEV-ONLY** | Used only by `seed.ts`. Production sandbox route uses real `git-sandbox.ts`. Acceptable. |
...
- **Root cause**: "dump the system prompt" — "dump" matched the extraction verb, but "system prompt" wasn't in the extraction noun list (only shadow-slVlNKhQwOpNNUCVwMQCkzMepMrDUhsOTViMN2dQ).
