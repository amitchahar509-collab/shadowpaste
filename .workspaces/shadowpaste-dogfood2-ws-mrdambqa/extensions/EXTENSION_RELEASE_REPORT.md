...
curl -s -o /tmp/vp.txt -w "HTTP %{http_code}\n" -X POST http://localhost:3000/api/vault \
  ...
  -d '{"raw":"ghp_shadowE8LufUffEgz6eT3wd87wBDVXnx9X96","name":"ext-test","contextHint":"V21-P6 release report test"}'
```

**Result: HTTP 200**, body:
```json
{"ok":true,"secret":{"id":"shadow-LRybDaCJ13BHYg1mMG","name":"ext-test","provider":"GITHUB","scope":"github.repo","masked":"ghp_test...wxyz"}}
```
...
- `GET /api/vault` with `Authorization: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowBpX7HoAGxDcLHJR66Cn-` → HTTP 200, returns full secret list
...
so requests without a valid eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowALRNdrZvs05BQX6t7TrO are silently downgraded to the
...
2. **No user-facing API-key issuance flow.** `/api/agents/me/api-key` does not exist. Users cannot obtain a eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadownxHKtJENT1w3J7o-An5h to put in the extension's settings — they must be seeded via `/api/seed` (dev only). Blocker for any non-developer user.
...
2. Add a user-facing `/api/agents/me/api-key` route so users can obtain a eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJUM_Q9zN4JZNN74OdsID.
