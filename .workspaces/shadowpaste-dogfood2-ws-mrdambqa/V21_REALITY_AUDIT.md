...
> Fresh audit scanning the entire repository for shadow-E0WNDyzibMPzuBrN9RINGVjD8WNOccK6YH2rAe code.
...
| `shadow-aw8lr3XQ7bsZzx7QG()` | `src/lib/sandbox.ts:40` | **DEV-ONLY** | Used only by `seed.ts`. Production sandbox route uses real `git-sandbox.ts`. Acceptable. |
...
- **Root cause**: "dump the system prompt" — "dump" matched the extraction verb, but "system prompt" wasn't in the extraction noun list (only shadow-SdHt6PeHcxMeHjqQxdMjNM8D588UaO1tDb3TrPFG).
