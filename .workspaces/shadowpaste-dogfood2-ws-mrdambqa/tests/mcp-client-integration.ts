...
async function callMcp(req: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await fetch(BASE, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadow14zKq72q4Hmg3j_E00CK" },
...
async function main() {
  ...
  const protectRes = await callMcp(rpc("tools/call", {
    ...
    arguments: {
      text: 'GITHUB_TOKEN=ghp_shadowv6gVJICLUJgoXm99RSpOwbuXTyAhf3\nSTRIPE_KEY=sk_test_shadowr9YDlj21kgL4bE9F2C9z\nAWS_KEY=AKIAWEE6HKE3W0LJS568',
      name: "env-file",
    },
  }))
  ...
  // Write proof log
  await Bun.write("/shadow-gHsybkmJn2yeqE7ratVbvNKVu54rGQ8Tb.log", log.join("\n"))
