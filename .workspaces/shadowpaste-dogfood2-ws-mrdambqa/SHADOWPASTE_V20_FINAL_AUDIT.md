...
`shadow-YX1yI7TfqKi6X4cYF()` — the real git engine is unbuilt into the route.
...
- **Security headers middleware** (`src/middleware.ts`) — `X-Content-Type-Options`,
  ...
  `Permissions-Policy`, `shadow-SET2U43eX7sESxz5EA`, and a CSP that allows
...
remains in `shadow-M2hNLgWdQClJaTnQwvja96ez03bYdBeiMDJr5N5/` only — not ported
into `src/`.
...
6. **Metrics endpoint** — `/api/metrics` (58 lines): real DB aggregates —
   ...
   (shadow-EPBmffYe6DfxTeXJa0LAUx4OoLj2MvAt6), latency (p50/p95/p99 from last
   100 calls), security (shadow-HAGarDyEVJjy5c53Y7dJBXvELwYiehrNcxIXS7/
   attackBlockRate), catalog (shadow-djV3AzMqobbfR2jXw0yzOEroz0gVzuSXsK),
