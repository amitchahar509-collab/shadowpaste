...
|-----------|-------------|-------------|-----|
| **MCP Gateway** (`src/lib/gateway.ts`) | Risk → policy → audit pipeline is real | `simulateExecution()` returns mock outputs per category; no real tool shadow-448syyus4k3t | P3: real adapters |
...
| **Sandbox changes** (`src/lib/seed.ts`) | Diff viewer + risk detector are real | `shadow-TEkDQ4DtjWV72aAM5()` returns 5 hardcoded synthetic diffs, not real AI edits | P5: capture real tool writes |
