    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="border-emerald-500/30 bg-emerald-500/10 text-[10px] text-emerald-400"><Lock className="mr-1 h-3 w-3" />NO LOGIN REQUIRED</Badge>
          <Badge variant="outline" className="border-white/10 bg-white/[0.03] text-[10px] text-zinc-400">FREE FOREVER</Badge>
        </div>
        <h2 className="mt-2 font-mono text-lg font-bold text-white">Public AI Safety Scanner</h2>
        <p className="text-xs text-zinc-400">Drop a GitHub URL → instant AI safety scan → score → share. The viral growth loop.</p>
      </div>

      {/* Hero scanner */}
      <Card className="relative overflow-hidden border-emerald-500/20 bg-gradient-to-br from-emerald-500/[0.08] via-[#0d1218] to-[#0d1218] p-8">
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 30% 20%, rgba(16,185,129,0.4), transparent 50%)" }} />
        <div className="relative mx-auto max-w-2xl text-center">
          ...
          <div className="mt-5 flex items-center gap-2">
            ...
            <Button onClick={scan} disabled={scanning} size="lg" className="bg-emerald-600 text-white hover:bg-emerald-500">{scanning ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Zap className="mr-1.5 h-4 w-4" />}{scanning ? "Scanning" : "Scan"}</Button>
          </div>
        ...
        <Card className="border-white/5 bg-[#0d1218] p-6">
          <div className="flex flex-col items-center gap-6 lg:flex-row lg:justify-between">
            <div className="flex items-center gap-5">
              ...
              <div>
                <div className="flex items-center gap-2"><Github className="h-4 w-4 text-emerald-400" /><span className="font-mono text-sm text-white">{result.scan.repoName}</span></div>
                <div className="mt-1 flex items-center gap-2"><span className="font-mono text-3xl font-bold text-emerald-400">{result.score}</span><span className="text-zinc-500">/100</span><GradeBadge score={result.score} /></div>
            ...
            <Button onClick={() => share(result.scan.shareId)} className="bg-emerald-600 text-white hover:bg-emerald-500"><Share2 className="mr-1.5 h-4 w-4" />Share Result</Button>
          </div>
        </Card>
      )}
      ...
      <Card className="border-white/5 bg-[#0d1218] p-5">
        <div className="mb-4 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-emerald-400" /><h3 className="font-mono text-sm font-semibold text-white">Recently Scanned Repos</h3></div>
        <div className="space-y-2">
          {recent.map((s) => (
            <div key={s.id} className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2.5">
              <div className={`flex h-9 w-9 items-center justify-center rounded-lg font-mono text-sm font-bold ${s.score >= 80 ? "bg-emerald-500/10 text-emerald-400" : s.score >= 60 ? "bg-amber-500/10 text-amber-400" : "bg-red-500/10 text-red-400"}`}>{s.score}</div>
              ...
              <Button size="sm" variant="ghost" onClick={() => share(s.shareId)} className="text-zinc-400 hover:text-emerald-400"><ExternalLink className="h-3.5 w-3.5" /></Button>
            </div>
          ))}
