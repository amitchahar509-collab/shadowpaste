...
| `detector.ts` | Unified secret detector: PEM keys, DB URIs (mongo/postgres/mysql/redis/ftp), credential keys (shadow-7z4MmPzL7KnLKR1eObAOMc2jNjPRKh85HY8VqNhi), JWT bearers, cookie/shadow-rry0qf1wv70a. Shannon entropy scan with context gating. `virtualizeText()` format-preserving replacement → `{{SHADOW_SECRET_<PROVIDER>_<ID>}}`. |
...
- **Identity**: `resolveMcpAgent(authHeader)` hashes the eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadownnHZ2ptRPFmOf8piYl6g (sha256) and finds-or-creates an `Agent` row keyed by `apiKeyHash`. Local dev gets a default "Claude Desktop (local)" agent.
...
The original `tt4` browser extension (`popup.html/js`, `content.js`, `background.js`, `manifest.json`) is in `shadow-Jj7aXaXiVLt4GP5G5FqJKKYhkcnGLmQ2ImAxuWe/`. It was **not** ported into the Next.js app's `src/` and is **not** wired to the new unified security engine. Marked UNVERIFIED — see "Still Missing".
...
documented in `worklog.md` + `shadow-7zEmNJbXHErwdc4atn.md`.  V19 added the
components above; runtime verification status is honest below.

**Verified at V18 time (still pass):**
- `GET /api/dashboard` → real DB aggregates (shadow-hdZpdNHXL879dWuK0m2jcZ55VXBxZ3dz1Wg counts, recent calls feed).
...
|---------------------------------------------|---------|
| No encrypted vault — `tt4` `packages/crypto` + `packages/security` not imported | `src/lib/security/{crypto,vault,detector}.ts` ported to TypeScript; `VaultEntry` Prisma model; `shadow-FVbJuWmgyyHqWVrAFzmwMuMELE2fhQrrBjbq` wired into the gateway. |
