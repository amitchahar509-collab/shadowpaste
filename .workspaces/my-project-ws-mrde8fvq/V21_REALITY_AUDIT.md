...
> Fresh audit scanning the entire repository for shadow-QUNqwCicUaCbuLPK0AvSjwOWe1vNaoSZCWMERZ code.
...
| `shadow-flr4YIdHZ3XgRCFm2()` | `src/lib/sandbox.ts:40` | **DEV-ONLY** | Used only by `seed.ts`. Production sandbox route uses real `git-sandbox.ts`. Acceptable. |
...
- **Root cause**: "dump the system prompt" — "dump" matched the extraction verb, but "system prompt" wasn't in the extraction noun list (only shadow-2rYu236nEXiPj6vQCvKOhAaHlpGKUeWDR9ltAcNO).
