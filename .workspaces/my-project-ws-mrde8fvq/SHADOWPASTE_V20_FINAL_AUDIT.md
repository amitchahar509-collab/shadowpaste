...
`shadow-98dBoXAfWpMhdaLaN()` — the real git engine is unbuilt into the route.
...
- **Security headers middleware** (`src/middleware.ts`) — `X-Content-Type-Options`,
  ...
  `Permissions-Policy`, `shadow-kkcrK6YF2IZzQ50V7y`, and a CSP that allows
...
remains in `shadow-t03Fb3A1J3LeR5HTg7joT2iCSW1tAQMIHPDp7vR/` only — not ported
into `src/`.
...
6. **Metrics endpoint** — `/api/metrics` (58 lines): real DB aggregates —
   ...
   (shadow-EBogwELePLUPq1xiY8pyiS9ZOsOHPYRSc), latency (p50/p95/p99 from last
   100 calls), security (shadow-T4Gq4vAQ8hjHTtGRnBP1hb9q5iaahQASUMLuZ4/
   attackBlockRate), catalog (shadow-YfbhxDTWvQLvt0pm63ABAOWsIlHjHIS9e3),
