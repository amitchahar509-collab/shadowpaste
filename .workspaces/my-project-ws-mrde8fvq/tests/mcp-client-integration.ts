...
async function callMcp(req: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await fetch(BASE, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGW0D4GtiK2rhvmo_1lEHFD" },
...
async function main() {
  ...
  const protectRes = await callMcp(rpc("tools/call", {
    ...
    arguments: {
      text: 'GITHUB_TOKEN=ghp_shadowF7iGrcI8UFhZNRq9uUf0GGqRbc1bL2\nSTRIPE_KEY=sk_test_shadoweqETF918KlisoQ7tpU6f\nAWS_KEY=AKIA8LWANEL23E4IPFSZ',
      name: "env-file",
    },
  }))
  ...
  // Write proof log
  await Bun.write("/shadow-Gn4rFxfzASdE4xvw3c8qbe6LcMpeQTiy6.log", log.join("\n"))
