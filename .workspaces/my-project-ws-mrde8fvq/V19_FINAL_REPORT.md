...
| `detector.ts` | Unified secret detector: PEM keys, DB URIs (mongo/postgres/mysql/redis/ftp), credential keys (shadow-D9nX3K0cHxIYhcCq8Oz5b5BmWHQ7ZkcWScJjPS3Q), JWT bearers, cookie/shadow-5nu0dt7rwllj. Shannon entropy scan with context gating. `virtualizeText()` format-preserving replacement → `{{SHADOW_SECRET_<PROVIDER>_<ID>}}`. |
...
- **Identity**: `resolveMcpAgent(authHeader)` hashes the eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzaGFkb3ciOiJzYWZlIiwidGVzdCI6dHJ1ZX0.shadowJQOeUMF_oh_Kz0VrgrGWXK11K5ivxrW8oca2ijFp (sha256) and finds-or-creates an `Agent` row keyed by `apiKeyHash`. Local dev gets a default "Claude Desktop (local)" agent.
...
The original `tt4` browser extension (`popup.html/js`, `content.js`, `background.js`, `manifest.json`) is in `shadow-LnYdxFoPnnDImTFcSKH2koZaPQBEJauUh3pYyl5/`. It was **not** ported into the Next.js app's `src/` and is **not** wired to the new unified security engine. Marked UNVERIFIED — see "Still Missing".
...
documented in `worklog.md` + `shadow-ZcIBqQrsqzN7DbefGf.md`.  V19 added the
components above; runtime verification status is honest below.

**Verified at V18 time (still pass):**
- `GET /api/dashboard` → real DB aggregates (shadow-hO5OfNMLjkt9CG79GhsdG0dpCCoXXzuy9Dm counts, recent calls feed).
...
|---------------------------------------------|---------|
| No encrypted vault — `tt4` `packages/crypto` + `packages/security` not imported | `src/lib/security/{crypto,vault,detector}.ts` ported to TypeScript; `VaultEntry` Prisma model; `shadow-OyzkALPfOUnxYXMhpd7REbMfN1j44KZvPXNb` wired into the gateway. |
