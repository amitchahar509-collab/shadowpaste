...
export function Agents() {
  ...
  const load = () => { setLoading(true); api<{ agents: Agent[] }>("/api/agents").then((d) => { setAgents(d.agents); if (d.agents[0] && !selected) setSelected(d.agents[0]); setLoading(false) }) }
  useEffect(() => { load() }, [])
  ...
  const create = async (data: { name: string; provider: string; description: string; trustScore: number; modelVersion: string }) => {
    await api("/api/agents", { method: "POST", body: JSON.stringify({ ...data, avatarColor: "#10b981" }) })
    toast.success("Agent created")
    setOpen(false)
    load()
  }

  if (loading) return <div className="grid gap-4 md:grid-cols-3">{Array.from({ length: 3 }).map((_, i) => <Card key={i} className="h-48 animate-pulse border-white/5 bg-white/[0.02]" />)}</div>

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-mono text-lg font-bold text-white">AI Agent Identities</h2>
          <p className="text-xs text-zinc-400">Every AI agent gets an identity, trust score, and full action history.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="bg-emerald-600 text-white hover:bg-emerald-500"><Plus className="mr-1.5 h-4 w-4" />Register Agent</Button></DialogTrigger>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-3 lg:col-span-2">
          {agents.map((a) => (
            <Card key={a.id} className={`cursor-pointer border bg-[#0d1218] p-5 transition-all ${selected?.id === a.id ? "border-emerald-500/40" : "border-white/5 hover:border-white/10"}`} onClick={() => setSelected(a)}>
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl font-mono text-lg font-bold text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${a.avatarColor}, ${a.avatarColor}99)` }}>
                  {a.name.charAt(0)}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                    ...
                    <Stat label="Allowed" value={a.allowedCalls} accent="emerald" />
                    <Stat label="Denied" value={a.deniedCalls} accent="red" />
                  </div>
                </div>
                <div className="shrink-0 text-center">
                  <div className={`font-mono text-2xl font-bold ${a.trustScore >= 80 ? "text-emerald-400" : a.trustScore >= 50 ? "text-amber-400" : "text-red-400"}`}>{a.trustScore}</div>
        ...
        {/* Detail panel */}
        <Card className="border-white/5 bg-[#0d1218] p-5 lg:sticky lg:top-24 lg:self-start">
          {selected ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <Progress value={selected.trustScore} className="h-2 bg-white/5 [&>div]:bg-gradient-to-r [&>div]:from-emerald-500 [&>div]:to-teal-400" />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button size="sm" variant={selected.status === "active" ? "default" : "outline"} onClick={() => setStatus(selected.id, "active")} className={selected.status === "active" ? "bg-emerald-600 text-white" : "border-white/10"}><ShieldCheck className="mr-1 h-3 w-3" />Active</Button>
              ...
              <div className="rounded-lg border border-white/5 bg-white/[0.02] p-3">
                <div className="mb-2 flex items-center gap-2"><Activity className="h-3.5 w-3.5 text-emerald-400" /><span className="text-[11px] font-semibold text-white">Identity Fingerprint</span></div>
    ...
    <DialogContent className="border-white/10 bg-[#0d1218]">
      ...
      <div className="space-y-3">
        ...
        <div><Label className="text-xs text-zinc-400">Provider</Label>
          <Select value={provider} onValueChange={setProvider}><SelectTrigger className="mt-1 border-white/10 bg-white/[0.03]"><SelectValue /></SelectTrigger><SelectContent className="border-white/10 bg-[#0d1218]">{PROVIDERS.map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select>
        </div>
        ...
        <Button className="w-full bg-emerald-600 text-white hover:bg-emerald-500" onClick={() => name && onCreate({ name, provider, description, trustScore: trust, modelVersion })}><Cpu className="mr-1.5 h-4 w-4" />Create Identity</Button>
      </div>
    </DialogContent>
  )
}
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = { active: "border-emerald-500/30 bg-emerald-500/10 text-emerald-400", suspended: "border-amber-500/30 bg-amber-500/10 text-amber-400", quarantined: "border-red-500/30 bg-red-500/10 text-red-400", revoked: "border-red-700/40 bg-red-700/15 text-red-300" }
