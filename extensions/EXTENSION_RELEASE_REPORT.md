...
```json
{"ok":true,"secret":{"id":"cmrc3jwhw0001wbzzxov7zegj","name":"ext-test","provider":"GITHUB","scope":"github.repo","masked":"ghp_test...wxyz"}}
```
...
- `GET /api/vault` with `Authorization: Bearer fake-key-not-real` → HTTP 200, returns full secret list
...
so requests without a valid bearer token are silently downgraded to the
...
2. **No user-facing API-key issuance flow.** `/api/agents/me/api-key` does not exist. Users cannot obtain a bearer token to put in the extension's settings — they must be seeded via `/api/seed` (dev only). Blocker for any non-developer user.
...
2. Add a user-facing `/api/agents/me/api-key` route so users can obtain a bearer token.
